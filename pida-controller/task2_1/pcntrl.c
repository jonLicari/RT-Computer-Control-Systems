...
void *Control(void *arg) {
	int k = 0;
	float ek, uk;	// Tracking error, actual control value
	...
	while (k<sample_num) {
		...
		//printf("\n%f", EtoR(ReadEncoder()));
		//printf("\n%f", mtr_pos);
		ek = ref[k] - mtr_pos;			// Calculate tracking error
		//printf("\n%f", ek);		
		uk = kp*ek;						// Control Value
		DtoA(VtoD(uk));
		theta[k] = mtr_pos;
		k++;
	}
		...
		switch(select) {
			...
			case 'u':
				...
				if (ip_ref == 's') {
					...
					scanf(" %d %f %f\n", &mag, &freq, &dc);
					Square(ref, MAXS, (float) Fs, (float) mag, freq, dc);
				}
				break;
